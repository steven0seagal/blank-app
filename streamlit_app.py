...
def personal_info_section():
    ...
    with col1:
        ...
        uploaded_file = st.file_uploader("Upload your professional photo", type=['png', 'jpg', 'jpeg'])
        ...
        if uploaded_file is not None:
            ...
            st.image(st.session_state.cv_data['photo'], caption="Current Photo", width=200)
    ...
    with col2:
        ...
        personal_info['full_name'] = st.text_input("Full Name", value=personal_info['full_name'])
        personal_info['title'] = st.text_input("Professional Title", value=personal_info['title'],
                                               placeholder="e.g., PhD Bioinformatician | Computational Biology Researcher")

        col2_1, col2_2 = st.columns(2)
        with col2_1:
            personal_info['email'] = st.text_input("Email", value=personal_info['email'])
            personal_info['phone'] = st.text_input("Phone", value=personal_info['phone'])

        with col2_2:
            personal_info['location'] = st.text_input("Location", value=personal_info['location'])
            personal_info['linkedin'] = st.text_input("LinkedIn", value=personal_info['linkedin'])
        ...
        personal_info['website'] = st.text_input("Personal Website", value=personal_info['website'])

        personal_info['summary'] = st.text_area("Professional Summary", value=personal_info['summary'],
                                                height=150,
...
def education_section():
    ...
    with st.expander("➕ Add New Education Entry"):
        with st.form("education_form"):
            ...
            institution = st.text_input("Institution", placeholder="e.g., University Name")
            location = st.text_input("Location", placeholder="City, Country")
            start_year = st.number_input("Start Year", min_value=1900, max_value=2030, value=2020)
            ...
            description = st.text_area("Description/Achievements",
    ...
    if education_list:
        ...
        for i, edu in enumerate(education_list):
            with st.container():
                ...
                st.write(f"📅 {edu['start_year']} - {edu['end_year']} | 📍 {edu['location']}")
                if edu['thesis_title']:
...
def experience_section():
    ...
    with st.expander("➕ Add New Work Experience"):
        with st.form("experience_form"):
            job_title = st.text_input("Job Title", placeholder="e.g., Senior Bioinformatics Analyst")
            company = st.text_input("Company/Institution", placeholder="e.g., Biotech Company")
            location = st.text_input("Location", placeholder="City, Country")
            start_date = st.date_input("Start Date")
            end_date = st.date_input("End Date")
            current_job = st.checkbox("This is my current position")
            job_type = st.selectbox("Job Type", ["Full-time", "Part-time", "Contract", "Internship", "Fellowship", "Postdoc"])
            description = st.text_area("Job Description & Achievements",
                                     height=150,
    ...
    if experience_list:
        ...
        for i, exp in enumerate(experience_list):
            with st.container():
                ...
                st.write(f"📅 {exp['start_date']} - {exp['end_date']} | 📍 {exp['location']} | 💼 {exp['job_type']}")
...
def skills_section():
    ...
    prog_langs_input = st.text_area("Programming Languages (one per line)",
    ...
    bioinfo_tools_input = st.text_area("Bioinformatics Tools (one per line)",
    ...
    stats_software_input = st.text_area("Statistical Software (one per line)",
    ...
    databases_input = st.text_area("Databases (one per line)",
    ...
    cloud_input = st.text_area("Cloud Platforms (one per line)",
    ...
    other_input = st.text_area("Other Technical Skills (one per line)",
...
def projects_section():
    ...
    with st.expander("➕ Add New Project"):
        with st.form("project_form"):
            ...
            project_type = st.selectbox("Project Type", ["Research Project", "Software Development", "Data Analysis", "Web Application", "Publication", "Other"])
            ...
            technologies = st.text_input("Technologies Used", placeholder="Python, R, Docker, AWS, etc.")
    ...
    with st.expander("➕ Add New Publication"):
        with st.form("publication_form"):
            ...
            pub_type = st.selectbox("Publication Type",
                                  ["Journal Article", "Conference Paper", "Book Chapter", "Preprint", "Poster", "Abstract"])
    ...
    if publications_list:
        ...
        for i, pub in enumerate(publications_list):
            with st.container():
                ...
                if st.button(f"🗑️ Remove", key=f"remove_pub_{i}"):
                    publications_list.pop(i)
                    st.rerun()
                st.divider()

def certifications_awards_section():
    st.markdown('<h2 class="section-header">🏆 Certifications & Awards</h2>', unsafe_allow_html=True)
        ...
        with st.form("certification_form"):
            ...
            if st.form_submit_button("Add Certification"):
                new_cert = {
                    ...
                    'expiry_date': "No Expiry" if no_expiry else expiry_date.strftime("%Y-%m-%d"),
    ...
    if certifications_list:
        for i, cert in enumerate(certifications_list):
            with st.container():
                ...
                st.write(f"📅 Issued: {cert['issue_date']} | Expires: {cert['expiry_date']}")
...
def generate_pdf_cv(cv_data, template="Professional Blue", page_format="A4"):
    ...
    try:
        ...
        buffer = BytesIO()
        doc = SimpleDocTemplate(buffer, pagesize=pagesize, rightMargin=0.75*inch, leftMargin=0.75*inch,
        ...
        colors_scheme = template_colors.get(template, template_colors["Professional Blue"])
        ...
        section_style = ParagraphStyle(
            ...
            borderColor=colors.HexColor(colors_scheme["border"]) if template != "Modern Minimal" else None,
        ...
        if contact_data:
            ...
            contact_table = Table(contact_table_data, colWidths=[1*inch, 2*inch, 1*inch, 2*inch])
        ...
        if cv_data['certifications']:
            ...
            for cert in cv_data['certifications']:
                ...
                cert_details = f"Issued: {cert['issue_date']} | Expires: {cert['expiry_date']}"
