...
def preview_export_section():
    ...
    if not personal.get("full_name"):
        st.warning(
            "Please complete your personal information before previewing or exporting."
        )
        return
    ...
    with st.container():
        ...
        if any(cv_data["skills"].values()):
            ...
            for category, skills_list in cv_data["skills"].items():
                if skills_list:
                    ...
                    st.write(
                        f"*{category_name}:* {', '.join(skills_list[:5])}{'...' if len(skills_list) > 5 else ''}"
                    )
        ...
        if cv_data["education"]:
            ...
            for edu in cv_data["education"]:
                st.write(
                    f"• {edu['degree']} - {edu['institution']} ({edu['start_year']}-{edu['end_year']})"
                )
    ...
    with col2:
        ...
        filename_json = (
            f"{personal['full_name'].replace(' ', '_')}_CV_data.json"
            if personal["full_name"]
            else "CV_data.json"
        )
        st.download_button(
    ...
    completion_items = [
        (
            ...
            bool(personal.get("full_name") and personal.get("email")),
        ),
    ...
    st.write(
        f"CV Completion: {completed}/{total} sections ({completion_percentage:.1f}%)"
    )
