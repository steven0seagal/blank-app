...
def publications_section():
    ...
    with st.expander("➕ Add New Publication"):
        with st.form("publication_form"):
            ...
            with col3:
                ...
                pmid = st.text_input(
                    "PMID (optional)",
                    help="PubMed ID if applicable"
                )

            with col4:
                url = st.text_input(
                    "URL/Link (optional)",
                    help="Direct link to the publication"
                )
    ...
    if publications_list:
        ...
        sorted_publications = sorted(publications_list, key=lambda x: x['year'], reverse=True)
        ...
        st.info("No publications added yet. Use the form above to add your academic publications and research papers.")
